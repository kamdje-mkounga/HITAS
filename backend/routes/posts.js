...
const getMediaType = (file) => {
  ...
  if (mime.includes('msword') || mime.includes('wordprocessingml') || ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx'].includes(ext)) {
    return 'document';
  }
  return 'file';
};
  ...
  try {
    ...
    const updatedPosts = await Promise.all(
      posts.map(async (post) => {
        ...
        if ((!post.mediaFiles || post.mediaFiles.length === 0) && post.mediaUrl) {
          post.mediaFiles = [
            {
              url: post.mediaUrl,
              path: post.mediaPath || '',
...
router.post('/', auth, (req, res) => {
  upload.array('media', MAX_FILES)(req, res, async (err) => {
    if (err) {
      console.error('Erreur Multer/Cloudinary:', err.message);
    ...
    try {
      ...
      if (!req.body.text?.trim() && (!req.files || req.files.length === 0)) {
        return res.status(400).json({ message: 'La publication doit contenir du texte ou au moins un fichier.' });
      }
      ...
      console.error('Erreur création post:', dbErr);
      res.status(500).json({ message: 'Erreur serveur lors de la création de la publication.' });
    }
  });
});
