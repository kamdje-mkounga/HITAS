    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 w-full overflow-hidden flex-1">
        ...
        <div className="mb-10 text-center sm:text-left overflow-hidden">
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-transparent dark:bg-gradient-to-r dark:from-slate-50 dark:via-indigo-200 dark:to-purple-400 dark:bg-clip-text break-words">
        ...
        <div className="bg-white/80 dark:bg-[#0b081e]/85 backdrop-blur-xl p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-indigo-900/60 shadow-2xl shadow-slate-200/50 dark:shadow-black/40 mb-12 transition-all duration-300 overflow-hidden">
          <div className="flex items-center gap-2.5 mb-6">
            ...
            <h2 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Partager un projet ou une experience</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {mediaPreviews.length > 0 && (
              <div className="bg-slate-100 dark:bg-[#030014]/60 border border-slate-200 dark:border-indigo-950/60 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 shadow-inner">
                {mediaPreviews.map((preview, index) => (
                  <div key={index} className="relative bg-white dark:bg-[#030014] rounded-xl border border-slate-200 dark:border-indigo-950 overflow-hidden flex flex-col p-2 shadow-md">
                    <button 
                      type="button" 
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-3 border-t border-slate-200 dark:border-indigo-950/60">
              <label className="flex items-center gap-2 cursor-pointer group text-xs text-slate-600 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                <span className="bg-white dark:bg-[#0b081e]/80 border border-slate-200 dark:border-indigo-900/60 group-hover:border-indigo-500/50 px-3 py-2 rounded-xl transition-all font-medium text-slate-700 dark:text-zinc-300 shadow-sm">
                  Choisir des fichiers
                </span>
                <span className="text-slate-500 dark:text-zinc-500 group-hover:text-slate-700 dark:group-hover:text-zinc-400 transition-colors">Images, PDF ou Vidéos (max 3 min)</span>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-3 bg-white/40 dark:bg-[#0b081e]/40 backdrop-blur-sm rounded-2xl">
            ...
            <div className="text-slate-500 dark:text-zinc-400 text-xs font-bold tracking-widest uppercase animate-pulse">Chargement de la galerie...</div>
          </div>
        ) : (
            ...
            {projects.map((project) => {
              ...
              return (
                <div 
                  ...
                  className="bg-white/85 dark:bg-[#0b081e]/85 backdrop-blur-xl p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-indigo-900/60 relative group transition-all duration-300 hover:border-indigo-700/50 shadow-xl shadow-slate-200/50 dark:shadow-black/40 overflow-hidden"
                >
                  ...
                  {editingId === project._id ? (
                    <div className="space-y-5 mt-2 bg-slate-50 dark:bg-[#030014]/60 backdrop-blur-sm p-5 rounded-xl border border-slate-200 dark:border-indigo-950/80 overflow-hidden">
                      <h4 className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-2">Modifier votre Portfolio</h4>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 block mb-1 uppercase tracking-wider">Titre du projet</label>
                          <input type="text" className="w-full bg-white dark:bg-[#0b081e]/80 border border-slate-200 dark:border-indigo-900/60 px-3 py-2 text-sm rounded-xl text-slate-900 dark:text-zinc-100 focus:outline-none focus:border-indigo-500/50 shadow-inner" value={editTitle} onChange={(e)=>setEditTitle(e.target.value)}/>
                        </div>
                        <div>
                          <label className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 block mb-1 uppercase tracking-wider">Technologies (Séparées par virgules)</label>
                          <input type="text" className="w-full bg-white dark:bg-[#0b081e]/80 border border-slate-200 dark:border-indigo-900/60 px-3 py-2 text-sm rounded-xl text-slate-900 dark:text-zinc-100 focus:outline-none focus:border-indigo-500/50 shadow-inner" value={editTechs} onChange={(e)=>setEditTechs(e.target.value)}/>
                        </div>
                      </div>

                      <div>
                        <label className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 block mb-1 uppercase tracking-wider">Description</label>
                        <textarea className="w-full bg-white dark:bg-[#0b081e]/80 border border-slate-200 dark:border-indigo-900/60 px-3 py-2 text-sm rounded-xl text-slate-900 dark:text-zinc-100 focus:outline-none focus:border-indigo-500/50 resize-none leading-relaxed shadow-inner" rows="3" value={editDescription} onChange={(e)=>setEditDescription(e.target.value)}/>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <input type="text" className="w-full bg-white dark:bg-[#0b081e]/80 border border-slate-200 dark:border-indigo-900/60 px-3 py-2 text-xs rounded-xl text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-indigo-500/50 shadow-inner" value={editGithub} onChange={(e)=>setEditGithub(e.target.value)} placeholder="GitHub URL"/>
                        <input type="text" className="w-full bg-white dark:bg-[#0b081e]/80 border border-slate-200 dark:border-indigo-900/60 px-3 py-2 text-xs rounded-xl text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-indigo-500/50 shadow-inner" value={editDemo} onChange={(e)=>setEditDemo(e.target.value)} placeholder="Démo Live URL"/>
                      </div>

                      {/* GESTION FINE DES FICHIERS EN ÉDITION */}
                      <div className="border-t border-slate-200 dark:border-indigo-950/60 pt-4">
                        <label className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 block mb-2 uppercase tracking-widest">📁 Gestion Fine des Fichiers</label>
                        
                        <div className="mb-4 bg-white dark:bg-[#030014] p-4 rounded-xl border border-slate-200 dark:border-indigo-950/60 shadow-inner">
                          <p className="text-[10px] text-slate-500 dark:text-zinc-400 mb-2.5">Fichiers sauvegardés :</p>
                          {existingMedia.length === 0 ? (
                            <p className="text-xs text-slate-400 dark:text-zinc-500 italic">Aucun fichier persistant.</p>
                          ) : (
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                              {existingMedia.map((mediaItem, idx) => (
                                <div key={idx} className="relative bg-slate-50 dark:bg-[#0b081e] rounded-lg border border-slate-200 dark:border-indigo-900/50 overflow-hidden flex flex-col p-2">
                                  <button 
                                    type="button" 
                                  ...
                                  <span className="text-[9px] text-slate-700 dark:text-zinc-300 truncate text-center">{getFileName(mediaItem)}</span>
                                </div>
                              ))}
                            </div>
                          )}
                        ...
                        <div className="bg-slate-100 dark:bg-[#0b081e]/80 p-4 rounded-xl border border-slate-200 dark:border-indigo-900/60 flex flex-col gap-3 shadow-inner">
                          <span className="text-[10px] text-slate-600 dark:text-zinc-400">Ajouter de nouveaux médias :</span>
                          <input type="file" multiple accept="image/*,video/*,application/pdf" onChange={handleEditFileChange} className="text-xs text-slate-600 dark:text-zinc-400 file:bg-white dark:file:bg-[#030014] file:text-slate-700 dark:file:text-zinc-300 file:border file:border-slate-300 dark:file:border-indigo-950 file:px-2.5 file:py-1 file:rounded-lg cursor-pointer hover:file:border-indigo-800 transition" />
                          ...
                          {editMediaPreviews.length > 0 && (
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-white dark:bg-[#030014] p-2 rounded-lg border border-indigo-500/20 mt-1">
                              {editMediaPreviews.map((preview, idx) => (
                                <div key={idx} className="relative bg-slate-50 dark:bg-[#0b081e] border border-amber-500/40 rounded-lg overflow-hidden flex flex-col p-2">
                                  ...
                                  <span className="text-[9px] text-amber-600 dark:text-amber-300 truncate text-center">{preview.name}</span>
                                </div>
                              ))}
                            </div>
                          )}
                      ...
                      <div className="flex gap-2 justify-end text-xs pt-3 border-t border-slate-200 dark:border-indigo-950/60">
                        <button onClick={() => { setEditingId(null); setEditMediaFiles([]); setEditMediaPreviews([]); setMediaToDelete([]); }} className="px-4 py-2 bg-transparent border border-slate-200 dark:border-indigo-900/60 text-slate-700 dark:text-zinc-400 rounded-xl hover:text-slate-900 dark:hover:text-white transition">Annuler</button>
                    ...
                    <div className="overflow-hidden">
                      ...
                      {project.technologies && project.technologies.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-5 overflow-hidden">
                          {project.technologies.map((tech, i) => (
                            <span key={i} className="bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 border border-indigo-500/20 text-[10px] px-2.5 py-0.5 rounded-md font-medium break-all">
                              {tech}
                            </span>
                          ))}
                        </div>
                      ...
                      {currentMedia && currentMedia.url && (
                        <div className="mb-6 overflow-hidden">
                          ...
                          <div className="flex items-center gap-2 bg-slate-100 dark:bg-[#030014] px-3 py-2 rounded-t-xl border-x border-t border-slate-200 dark:border-indigo-900/40 text-xs text-indigo-600 dark:text-indigo-200">
                            <span>📎</span>
                          ...
                          <div className="rounded-b-xl overflow-hidden border border-slate-200 dark:border-indigo-900/40 bg-slate-100 dark:bg-[#030014]/60 h-[260px] sm:h-[400px] w-full flex items-center justify-center relative shadow-inner">
                            {currentMedia.type === 'video' ? (
                              ...
                              <div className="w-full h-full flex flex-col items-center justify-center bg-white/40 dark:bg-[#0b081e]/40 p-6 text-center">
                                ...
                                <p className="text-xs text-slate-600 dark:text-zinc-400 mb-4 font-medium truncate max-w-xs">{currentFileName}</p>
                                <a 
